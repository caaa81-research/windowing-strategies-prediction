...
# XGBoost model: hyperparameter tuning with TimeSeriesSplit + early stopping
#
# Authors:
#   Carlos A. del Álamo-Alonso  (Universidad Loyola Andalucía)
#   Antonio Manuel Durán-Rosal  (Universidad Loyola Andalucía)
#   Javier Pérez-Rodríguez      (Universidad de Córdoba)
#
...
def build_preprocessor(X: pd.DataFrame):
    ...
    return ColumnTransformer(
        transformers=[("num", "passthrough", num_cols), ("cat", cat_tf, cat_cols)],
        remainder="drop",
...
# -----------------------------------------------------------------------------
def get_param_space_xgb():
    return {
        "eta":              [0.02, 0.03, 0.05, 0.08],
        "max_depth":        [3, 4, 5, 6],
...
# -----------------------------------------------------------------------------
def score_params_tss_xgb(X: pd.DataFrame, y: np.ndarray, tss: TimeSeriesSplit, params: dict):
    rows = []
    ...
    core_params = {
        ...
        "tree_method": "hist",
        "seed":        RANDOM_STATE,
        "nthread":     0 if N_JOBS == -1 else N_JOBS,
    }
    ...
    for fold, (tr, te) in enumerate(tss.split(X), start=1):
        ...
        dvalid = xgb.DMatrix(X_te_t, label=y_te_log)
        ...
        pred_log = bst.predict(dvalid, iteration_range=(0, bst.best_iteration + 1))
        ...
        rows.append({
            ...
            "best_iteration": int(bst.best_iteration), "n_test": int(len(te)),
        })
...
# 8) TUNING LOOP
# -----------------------------------------------------------------------------
def tune_timeseries_xgb(df_in: pd.DataFrame, feature_drop: list, model_label: str):
    ...
    candidates = list(ParameterSampler(get_param_space_xgb(), n_iter=N_ITER, random_state=RANDOM_STATE))
    best       = {"rmse_log": np.inf, "params": None, "folds": None}

    print("\n" + "=" * 80)
    print(f"TUNING XGBoost: {model_label}")
    print(f"n_splits={N_SPLITS} | n_iter={N_ITER} | early_stopping={EARLY_STOPPING_ROUNDS}")
    print("=" * 80)

    for i, p in enumerate(candidates, start=1):
        mean_rmse_log, fold_df = score_params_tss_xgb(X, y, tss, p)
        best_iter_mean = fold_df["best_iteration"].mean()
    ...
    summary = {
        ...
        "best_iter_mean": float(folds["best_iteration"].mean()),
...
# -----------------------------------------------------------------------------
params_std, summary_std, folds_std = tune_timeseries_xgb(df, DROP_STANDARD, "STANDARD (no windows)")
params_ext, summary_ext, folds_ext = tune_timeseries_xgb(df, DROP_EXTENDED, "EXTENDED (with windows)")
...
# 10) SAVE OUTPUTS
# -----------------------------------------------------------------------------
final_table.to_csv("final_comparable_table_tss_es.csv", index=False)
folds_std.to_csv("folds_standard_tss_es.csv", index=False)
folds_ext.to_csv("folds_extended_tss_es.csv", index=False)
pd.Series(params_std).to_csv("best_params_standard_tss_es.csv")
pd.Series(params_ext).to_csv("best_params_extended_tss_es.csv")
